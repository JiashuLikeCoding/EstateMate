...
struct CRMContactTasksListView: View {
    ...
    var body: some View {
        EMScreen {
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    EMSectionHeader("待办任务", subtitle: "该客户的跟进提醒")
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                NavigationLink {
                    CRMTaskEditView(mode: .create(contactId: contactId))
                } label: {
    ...
    private func reload() async {
        ...
        do {
            tasks = try await service.listTasks(contactId: contactId, includeDone: includeDone)
        } catch {
    ...
    private func toggleDone(task: CRMTask) async {
        do {
            let updated = try await service.updateTask(
                id: task.id,
                patch: CRMTaskUpdate(contactId: task.contactId, title: nil, notes: nil, dueAt: nil, isDone: !task.isDone)
            )
...
private struct ContactTaskCard: View {
    @Environment(\.emAccentColor) private var accent
            ...
            HStack(alignment: .top, spacing: 12) {
                Button {
                    ...
                    Image(systemName: task.isDone ? "checkmark.circle.fill" : "circle")
                        .font(.title3)
                        .foregroundStyle(task.isDone ? accent : EMTheme.ink2)
                }
                .buttonStyle(.plain)
...
#Preview {
    NavigationStack {
        CRMContactTasksListView(contactId: UUID())
    }
}
